...
@commands('alerts')
@example('.alerts')
@example('.alerts London')
@example('.alerts 90210')
def weather_alerts(bot, trigger):
    """Show active weather alerts/watches/warnings for a location (US only)."""
    if bot.config.weather.geocoords_api_key is None or bot.config.weather.geocoords_api_key == '':
        return bot.reply("GeoCoords API key missing. Please configure this module.")
    ...
    try:
        ...
        bot.reply("Could not find location: " + str(err))
        return

    # Query National Weather Service API for alerts
    # NWS API is US-only, so this will only work for US locations
    nws_url = f"https://api.weather.gov/alerts/active?point={latitude},{longitude}"
    
    try:
        headers = {'User-Agent': 'Sopel IRC Bot Weather Module', 'Accept': 'application/geo+json'}
        r = requests.get(nws_url, headers=headers, timeout=10)
        
        if r.status_code != 200:
            return bot.say(f"{location}: Unable to fetch alerts (possibly not in US/territories)")
        
        data = r.json()
        features = data.get('features', [])
        
        if not features:
            return bot.say(f"{location}: No active weather alerts")
        ...
        for feature in features[:3]:  # Limit to first 3 alerts
            ...
            # Format severity with color/emphasis indicators
            severity_indicator = {
                'Extreme': '🔴',
                'Severe': '🟠', 
                'Moderate': '🟡',
        ...
        if alerts:
            ...
            return bot.say(alert_text)
        else:
            return bot.say(f"{location}: No active weather alerts")
            
    except requests.exceptions.Timeout:
        return bot.say(f"{location}: Request timed out while fetching alerts")
    except requests.exceptions.RequestException as e:
        LOGGER.error(f"Error fetching weather alerts: {e}")
        return bot.say(f"{location}: Error fetching weather alerts")
    except Exception as e:
        LOGGER.error(f"Unexpected error in weather alerts: {e}")
        return bot.say(f"{location}: Unable to process weather alerts")
